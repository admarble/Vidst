...
setup_workflows() {
  ...
  # Copy workflow files
  cp ./workflows/branch-validator.yml "$WORKFLOW_DIR/"
  cp ./workflows/commit-validator.yml "$WORKFLOW_DIR/"
  cp ./workflows/project-board-automation.yml "$WORKFLOW_DIR/"
...
setup_issue_templates() {
  ...
  cp ./templates/feature_request.md "$TEMPLATE_DIR/"
  cp ./templates/bug_report.md "$TEMPLATE_DIR/"
  cp ./templates/component_replacement.md "$TEMPLATE_DIR/"
...
setup_pr_template() {
  ...
  # Copy PR template
  cp ./templates/pull_request_template.md "$PR_TEMPLATE"
...
See the [Project Management documentation](./refactor/project_management) for details.
...
if [ ! -f "./README.md" ] || [ ! -d "./workflows" ]; then
  echo -e "${RED}Error: Must run this script from the project_management directory!${NC}"
  exit 1
fi
...
echo "3. Review the project management documentation in ./docs/"
...
echo ""
echo "For more information, see the project management README.md"
